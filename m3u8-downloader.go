...
func downloadTsFile(ts TsInfo, download_dir, key string, retries int) {
	defer func() {
		if r := recover(); r != nil {
			//fmt.Println("The network is unstable and downloading at breakpoint is ongoing")
...
func downloader(tsList []TsInfo, maxGoroutines int, downloadDir string, key string) {
	retry := 5 // Single ts download retry times
	var wg sync.WaitGroup
	limiter := make(chan struct{}, maxGoroutines) // chan struct memory usage 0 bool usage 1
	tsLen := len(tsList)
	downloadCount := 0
